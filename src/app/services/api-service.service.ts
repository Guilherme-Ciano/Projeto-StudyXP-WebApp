...
export class ApiServiceService {
  ...
  async BuscarAluno() {
    ...
    let aluno = this.alunos.find(
      (element) => element.ra === this.user_Aluno.RA
    );
    return aluno;
  }
  ...
  async BuscarProf() {
    await axios
      .get('http://localhost:9090/professores/index')
      .then((response) => {
    ...
    let prof = this.professores.find(
      (element) =>
        element.email === this.user_Prof.Email &&
        element.nome === this.user_Prof.Nome
    );
    return prof;
  }
  ...
  async EnviarTarefa(id: number) {
    ...
    await axios
      .post(
        'http://localhost:9090/alunos/tarefas/concluirtarefa?id=' + id,
        concluir
      )
      .then((response) => {
  ...
  async AlterarDadosAluno(id: number, newData) {
    await axios
      .post('http://localhost:9090/alunos/update?id=' + id, newData)
      .then((response) => {
        this.snbar.success(response.data.status, response.data.message);
        this.refresh();
      });
